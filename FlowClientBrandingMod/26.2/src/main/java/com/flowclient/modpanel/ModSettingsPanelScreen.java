...
public abstract class ModSettingsPanelScreen<T> extends ModPanelScreen {
    ...
    protected void addHudPositionButton(int frameX, int frameY, int frameWidth, int visibleHeight, int rowIndex, HudElement element) {
        ...
        this.addRenderableWidget(new ModPanelButton(
                ...
                () -> this.minecraft.gui.setScreen(new HudModSettingsScreen(this, element))
        ));
    }
    ...
    private void drawSectionHeaders(GuiGraphicsExtractor graphics, int frameX, int frameY, int frameWidth, int frameHeight) {
        for (int i = 0; i < this.rows.size(); i++) {
            ModSettingRow<T> row = this.rows.get(i);
            if (!row.header()) {
                continue;
            ...
            graphics.text(this.font, row.title(), frameX + FRAME_INSET, y + 6, ModPanelTheme.ACCENT_BLUE);
        }
    }

    private void drawRowSeparators(GuiGraphicsExtractor graphics, int frameX, int frameY, int frameWidth, int frameHeight) {
