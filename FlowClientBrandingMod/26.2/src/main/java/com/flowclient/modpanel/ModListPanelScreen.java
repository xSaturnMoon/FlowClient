...
public final class ModListPanelScreen extends ModPanelScreen {
        ...
        for (int index = 0; index < visibleEntries.size(); index++) {
            ...
            if (entry.hasDedicatedSettingsScreen()) {
                this.addRenderableWidget(ModSettingsButtons.create(
                        ...
                        () -> this.minecraft.gui.setScreen(entry.openSettings(this))
                ));
            }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        if (this.listWidget != null && this.listWidget.contains(mouseX, mouseY)) {
            this.scrollOffset = (int) Math.max(0, this.scrollOffset - scrollY * ModEntryListWidget.ROW_HEIGHT);
            this.rebuildWidgets();
            return true;
        }
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        for (int index = 0; index < visibleEntries.size(); index++) {
            ...
            graphics.text(font, Component.literal(entry.displayName()), textX, textY, ModPanelTheme.TEXT);

            if (showCategoryBadge) {
                String badge = " · " + entry.category().label();
                int badgeX = textX + font.width(entry.displayName());
                graphics.text(font, badge, badgeX, textY, ModPanelTheme.TEXT_MUTED);
            }
        }
    ...
    private void drawPanelFrame(GuiGraphicsExtractor graphics, int x, int y, int width, int height) {
    ...
    private void drawFooter(GuiGraphicsExtractor graphics, List<ModEntry> visibleEntries) {
        Font font = Minecraft.getInstance().font;
        String footer;
        ...
        graphics.text(font, footer, this.listWidget.x(), this.listWidget.footerTextY(), ModPanelTheme.TEXT_MUTED);
    }
