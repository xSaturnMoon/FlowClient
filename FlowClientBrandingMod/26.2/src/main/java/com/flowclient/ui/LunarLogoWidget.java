...
public final class LunarLogoWidget {
     ...
     */
    public static void drawLogoAt(GuiGraphicsExtractor g, int left, int topY, int size) {
        drawEmblem(g, left, topY, size);
    }

    public static void drawLogo(GuiGraphicsExtractor g, int centerX, int topY, int size) {
        int left = centerX - size / 2;
        drawEmblem(g, left, topY, size);
    }

    private static void drawEmblem(GuiGraphicsExtractor g, int left, int topY, int size) {

        // ── Background container (rounded square, semi-dark) ──────────────────
        int bgDark = ARGB.color(200, 12, 14, 22);
        int glow   = ARGB.color(60, 99, 163, 255);
        ...
        int white = ARGB.color(255, 240, 245, 255);
        ...
        int starColor = ARGB.color(255, 200, 215, 255);
    ...
    private static void fillCircle(GuiGraphicsExtractor g, int cx, int cy, int r, int color) {
