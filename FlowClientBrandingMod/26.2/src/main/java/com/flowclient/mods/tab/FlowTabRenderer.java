...
public final class FlowTabRenderer {
    ...
    public static void render(FlowTabBridge bridge, GuiGraphicsExtractor graphics, int screenWidth, Scoreboard scoreboard, Objective objective) {
    ...
    private static void drawPanel(
        ...
        if (!layout.headerLines().isEmpty()) {
            ...
            for (FormattedCharSequence line : layout.headerLines()) {
                ...
                graphics.text(font, line, lineX, cursorY, -1, true);
                cursorY += ROW_HEIGHT;
            }
            cursorY++;
        }
    ...
    private static void drawPlayerRow(
        ...
        graphics.text(font, name, drawX, rowY, -1, true);
    ...
    private static void drawHead(GuiGraphicsExtractor graphics, Minecraft minecraft, PlayerInfo info, int x, int y) {
        ...
        PlayerFaceExtractor.extractRenderState(graphics, skin, x, y, HEAD_SIZE, info.showHat(), upsideDown, -1);
    }
    ...
    private static Comparator<PlayerInfo> comparatorFor(FlowTabSort sort) {
        return switch (sort) {
            case NAME_ASC -> Comparator.comparing(p -> p.getProfile().name(), String.CASE_INSENSITIVE_ORDER);
            case NAME_DESC -> Comparator.comparing((PlayerInfo p) -> p.getProfile().name(), String.CASE_INSENSITIVE_ORDER).reversed();
            ...
            case VANILLA -> Comparator.comparingInt(PlayerInfo::getTabListOrder);
        };
    }
