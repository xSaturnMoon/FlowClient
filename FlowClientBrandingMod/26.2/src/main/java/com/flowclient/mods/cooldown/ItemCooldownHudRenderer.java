...
public final class ItemCooldownHudRenderer {
    ...
    public static void render(Minecraft mc, GuiGraphicsExtractor graphics, Font font, boolean preview) {
        ...
        float itemCooldown = preview ? 0.35f : (mainHand.isEmpty()
                ? 0.0f
                : player.getCooldowns().getCooldownPercent(mainHand, partialTick));
    ...
    private static void drawBar(GuiGraphicsExtractor graphics, int x, int y, int width, float fill, int color) {
        graphics.fill(x, y, x + width, y + BAR_HEIGHT, ARGB.color(255, 30, 30, 30));
        int filled = Math.max(1, (int) (width * Math.max(0.0f, Math.min(1.0f, fill))));
        graphics.fill(x, y, x + filled, y + BAR_HEIGHT, color);
        graphics.fill(x, y, x + width, y + 1, ARGB.color(255, 0, 0, 0));
        graphics.fill(x, y + BAR_HEIGHT - 1, x + width, y + BAR_HEIGHT, ARGB.color(255, 0, 0, 0));
    }
}
