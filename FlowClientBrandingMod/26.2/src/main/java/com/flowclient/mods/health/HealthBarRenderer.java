...
public final class HealthBarRenderer {
    ...
    public static void renderPreview(GuiGraphicsExtractor graphics, Font font, int centerX, int centerY) {
        HealthBarSettings settings = HealthBarSettings.get();
    ...
    private static void drawBar(
        ...
        if (settings.showNumeric()) {
            ...
            graphics.text(font, Component.literal(text), centerX - textWidth / 2, barY - font.lineHeight - 1, 0xFFFFFFFF, true);
        }
    ...
    private static void drawGradient(GuiGraphicsExtractor graphics, int x, int y, int width, int height, float ratio) {
    ...
    private static void drawOutlined(GuiGraphicsExtractor graphics, int x, int y, int width, int height, float ratio) {
        graphics.fill(x - 1, y - 1, x + width + 1, y + height + 1, 0xFF000000);
        drawClassic(graphics, x, y, width, height, ratio, 0xCC202028, false);
    }

    private static void drawSegments(GuiGraphicsExtractor graphics, int x, int y, int width, int height, float ratio) {
        int segments = 10;
        int gap = 1;
