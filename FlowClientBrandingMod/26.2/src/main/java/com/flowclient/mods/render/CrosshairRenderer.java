...
public final class CrosshairRenderer {
    ...
    public static void render(Minecraft mc, GuiGraphicsExtractor graphics) {
        CrosshairSettings settings = CrosshairSettings.get();
        if (!settings.showWhileDebug() && mc.getDebugOverlay().showDebugScreen()) {
            return;
        }
    ...
    private static void drawRing(GuiGraphicsExtractor graphics, int cx, int cy, int outer, int thick, int color) {
    ...
    private static void hBar(GuiGraphicsExtractor graphics, int cx, int cy, int size, int thick, int gap, int color) {
        graphics.fill(cx - size, cy - thick, cx - gap, cy + thick + 1, color);
        graphics.fill(cx + gap + 1, cy - thick, cx + size + 1, cy + thick + 1, color);
    }

    private static void vBar(GuiGraphicsExtractor graphics, int cx, int cy, int size, int thick, int gap, int color) {
