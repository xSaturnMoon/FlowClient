    ...
    private static boolean hasGeyserTextureSignal(PlayerInfo info, GameProfile profile) {
        if (info != null) {
            try {
                ...
                if (skin != null) {
                    if (isGeyserTextureUrl(textureUrl(skin.body()))) return true;
                    if (isGeyserTextureUrl(textureUrl(skin.cape()))) return true;
                    if (isGeyserTextureUrl(textureUrl(skin.elytra()))) return true;
                }
