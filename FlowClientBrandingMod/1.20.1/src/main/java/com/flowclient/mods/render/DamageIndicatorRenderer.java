    ...
    public static void render(Minecraft client, GuiGraphics graphics, Font font) {
        ...
        for (DamageIndicatorTracker.DamagePopup popup : DamageIndicatorTracker.popups()) {
            ...
            Vec3 anchor = entity != null
                    ...
                    : popup.worldPosition();
            Vec3 projected = WorldProjection.projectPointToScreen(client, anchor.add(0.0D, popup.rise(), 0.0D));
            ...
            String text = popup.healing()
                    ...
                    : formatAmount(popup.amount());
            x -= font.width(text) / 2;
            FlowGfx.text(graphics, font, Component.literal(text), x, y, color, true);
        }
    }
