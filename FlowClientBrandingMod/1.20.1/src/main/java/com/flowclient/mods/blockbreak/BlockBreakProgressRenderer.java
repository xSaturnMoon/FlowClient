...
public final class BlockBreakProgressRenderer {
    ...
    private static void draw(
        ...
        if (settings.showPercentage()) {
            FlowGfx.text(graphics, font, formatPercent(progress), x + PADDING, contentY, 0xFFFFFFFF, true);
        ...
        graphics.fill(barX, barY, barX + barWidth, barY + BAR_HEIGHT, FlowArgb.color(255, 30, 30, 30));
        int fillWidth = Math.max(1, (int) (barWidth * progress));
        graphics.fill(barX, barY, barX + fillWidth, barY + BAR_HEIGHT, FlowArgb.color(255, 76, 175, 80));
        graphics.fill(barX, barY, barX + barWidth, barY + 1, FlowArgb.color(255, 0, 0, 0));
        graphics.fill(barX, barY + BAR_HEIGHT - 1, barX + barWidth, barY + BAR_HEIGHT, FlowArgb.color(255, 0, 0, 0));
        graphics.fill(barX, barY, barX + 1, barY + BAR_HEIGHT, FlowArgb.color(255, 0, 0, 0));
        graphics.fill(barX + barWidth - 1, barY, barX + barWidth, barY + BAR_HEIGHT, FlowArgb.color(255, 0, 0, 0));
    }
