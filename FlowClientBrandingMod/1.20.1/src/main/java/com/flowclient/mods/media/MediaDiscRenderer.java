...
final class MediaDiscRenderer {
    ...
    static void draw(GuiGraphics graphics, int x, int y, int size, float rotation, MediaAlbumArtTexture album) {
    ...
    private static void drawGpuDisc(
        ...
        pose.translate(-size / 2.0F, -size / 2.0F, 0.0D);
        graphics.blit(album.gpuTextureLocation(), 0, 0, size, size, 0, 0, size, size, size, size);
        pose.popPose();
    ...
    private static void drawCpuDisc(
        ...
        pose.translate(-renderSize / 2.0F, -renderSize / 2.0F, 0.0D);
    ...
    private static void drawRotatedAlbumArt(
        ...
        for (int dy = -radius; dy < radius; dy += step) {
            for (int dx = -radius; dx < radius; dx += step) {
                ...
                int color = toRenderColor(image.getPixelRGBA(sx, sy));
                if (color == 0) {
                    continue;
                }
    ...
    private static int toRenderColor(int pixel) {
        ...
        return FlowArgb.color(alpha, red, green, blue);
    }
}
