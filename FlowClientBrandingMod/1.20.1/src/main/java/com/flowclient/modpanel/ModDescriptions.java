...
public final class ModDescriptions {
    ...
    public static void draw(GuiGraphics graphics, Font font, String text, int x, int y, int maxWidth) {
        if (text == null || text.isBlank()) {
            return;
        }
        ...
        for (String line : wrap(font, text, maxWidth)) {
            FlowGfx.text(graphics, font, Component.literal(line), x, lineY, TEXT_COLOR, false);
