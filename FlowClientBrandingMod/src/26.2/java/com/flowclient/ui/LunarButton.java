...
public final class LunarButton extends AbstractButton {
    ...
    protected void extractContents(GuiGraphicsExtractor g, int mx, int my, float pt) {
        ...
        if (style == Style.STORE) {
            bgColor = hov ? ARGB.color(255, 22, 101, 52) : ARGB.color(220, 16, 64, 36);
            textColor = ARGB.color(255, 74, 222, 128);
            borderColor = ARGB.color(255, 34, 197, 94);
        } else if (style == Style.QUEST_ACTION) {
            bgColor = hov ? ARGB.color(255, 163, 230, 53) : ARGB.color(255, 132, 204, 22);
            textColor = ARGB.color(255, 15, 23, 42);
            radius = 6;
        } else {
            bgColor = hov ? ARGB.color(255, 45, 52, 70) : ARGB.color(220, 22, 26, 36);
            textColor = hov ? ARGB.color(255, 255, 255, 255) : ARGB.color(255, 200, 208, 220);
        }
        ...
        if (style == Style.DEFAULT) {
            int accent = hov ? ARGB.color(255, 99, 163, 255) : ARGB.color(180, 66, 120, 200);
        ...
        g.text(font, label, textX, textY, textColor);

        if (badgeText != null) {
            int bx = textX + textW + 6;
            int by = getY() + (height - 13) / 2;
            UiColors.fillRounded(g, bx, by, badgeW, 13, 4, ARGB.color(180, 40, 50, 70));
            g.text(font, badgeText, bx + 5, by + (13 - font.lineHeight) / 2 + 1, ARGB.color(255, 148, 163, 184));
        }
    }
